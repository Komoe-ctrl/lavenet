...
// apps/api's tsconfig (decorators enabled) instead of the workspace root
// one, then hands off to the actual TS script. Kept separate so the env var
// is set before the register hook reads it — setting it from within the
// TS entry file itself would be too late.
...
process.env.SWC_NODE_PROJECT = path.resolve(__dirname, '../../apps/api/tsconfig.app.json');
